...
doc_ids_file = "processed_CLERC/docIds.txt"
with open(doc_ids_file, 'r') as file:
    valid_doc_ids = set(line.strip() for line in file)
...
for idx, sample in enumerate(dataset):
    ...
    if first_value in valid_doc_ids:
        ...
        print(f"Processing datapoint {idx + 1}: doc_id={first_value}")

        # Add to batch
        batch_texts.append(second_key + second_value) # Concatenate heading and text to embed
        batch_metadatas.append({
            "doc_id": first_value,
            "doc_heading": second_key,
            "doc_text": second_value
        })
        total_processed += 1
